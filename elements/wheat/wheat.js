...
function lerpColor(a, b, amount) {
  ...
  class Plant {
    constructor(x, y, l) {
      ...
      this.max_thickness = 15; //thickness of straw
      this.thickness =
        Util.map(this.length, H, 0, this.min_thickness, this.max_thickness) *
        Util.random(0.8, 1.2);
      this.z_value = 0;
      this.colors = ["#e89348", "#fac553"]; //4a2525
      this.true_color = this.colors[
        Math.floor(Math.random() * this.colors.length)
      ];
    }
    ...
    init() {
      ...
      for (let i = 1; i < this.resolution; i++) {
        ...
        //this.leaves.push(leaf); //THIS REMOVED
  ...
  let plants = [];
  let gravity = new Vector(0, 0.8); //GRAVITY
  let wind = {
    ...
    reset: function() {
      ...
      this.goal = Util.random(-0.1, 0.1); //WIND STRENGTH
    },
    update: function() {
  ...
  let backgroundColor = "#ffebcd"; //WHAT COLOR IT COVERS OLD DRAWING WITH
  
  function populate() {
    plants = [];
    let div_x = 9, //AMOUNT STRAW
      div_y = 9;
    let total_div = div_x * div_y;
  
    let part_x = 4;// W / div_x;
  
    for (let i = 0; i < total_div; i++) {
      //offset_x = (i % div_x) * part_x,
      let offset_x = Util.random(0,W),
        offset_y = 3,//Math.floor(i / div_x), //COLOR VAR ADJUSTMENT
        z_value = Util.map(offset_y, 0, div_y - 1, 0.8, 0),
        p_length = Util.map(offset_y, 0, div_y - 1, H * 0.4, H); //HEIGHT ADJUSTMENT
